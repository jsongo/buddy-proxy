...
def test_badcase_20260905_regression():
    ...
    if calls:
        ...
        check("b1截断: path 参数完整", a1.get("path") == "/home/user/project/src/buddy_proxy/zcode_provider.py", f"path={a1.get('path')!r}")
    ...
    # 注：下文 payload 中的 chat-id 是测试用的无意义 ID，不指向任何真实的群
    # 或系统。此处只用来复现「command 里嵌 python 脚本、markdown 转义 \$ 是
    # JSON 非法转义（Invalid \escape）」这一解析缺陷，与 ID 或数据本身无关。
